...
- **Footer**: scuro (`--bg-black`), 4 colonne (brand + nav + prodotto + azienda), riga legale. Struttura riconciliata con le pagine reali il 2026-08-17: **Navigazione** rispecchia esattamente le 5 voci dell'header (`Header.tsx`, stesso ordine e stessa label "Perchè AXON") + Shop; **Prodotto** ha solo voci con destinazione propria — AXON KIT (`/shop`), Come si usa (`/#come-indossare`), FAQ (`/#faq`), Testimonianze (`/#testimonianze`) — niente più AXON SHELL duplicato su `/shop` né FAQ doppiata con Navigazione; **Azienda** è Contatti (`/contatti`) + `info@axon-tech.it` (mailto) + la ragione sociale "Axon-Tech S.r.l." come testo semplice (non è più un link `#`: non ha una destinazione); riga legale Privacy/Cookie/Termini punta a `/privacy` `/cookie` `/termini` via `next/link` invece dei vecchi stub `#`. Icone social: rese solo quando `SOCIAL.<piattaforma>` in `lib/links.ts` non è più `'#'` (rendering condizionato sul valore, non commentato/hardcoded) — finché tutti e tre restano placeholder, il blocco social non renderizza nulla, evitando link morti; impostare l'URL reale in `lib/links.ts` è un one-line restore, nessuna modifica a `Footer.tsx`. Stessa regola applicata al link Instagram del box "Contatti diretti" su Contatti (vedi §10).
...
**Contatti riallineato (2026-08-17)** — entrambe le `Section` (hero `id="top"` e form `id="contatto"`) sono passate a `background="black"`: prima l'hero ereditava il trasparente `--bg` di default e il form viveva su `background="elevated"`, uniche pagine utility a non usare nero pieno. Allineati anche due dettagli di drift rispetto a `ProfessionistiContent`: l'heading "Contatti diretti" del box contatti ora usa `className="text-h3"` invece di uno style inline `fontSize: var(--fs-h3)` (stesso valore visivo, ma eredita anche letter-spacing/weight dalla classe condivisa); il link Instagram del box contatti è reso solo quando `SOCIAL.instagram !== '#'`, senza più il fallback grigio non cliccabile — stessa regola del Footer (§5). Lead paragraph, spaziatura dei campi form (`gap-5`, griglia `sm:grid-cols-2`) e il Card di stato "successo" (`p-8`, label bold a `--fs-h3`, corpo `--fs-body`/`--text-muted`) erano già identici a Professionisti/Aziende, nessuna modifica necessaria lì.
