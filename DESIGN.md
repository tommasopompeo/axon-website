...
**Box "Contatti diretti" rimosso (2026-08-17)** — il layout a due colonne (form + Card con email/indirizzo/Instagram) è tornato a colonna singola: il form ora vive in un `<div className="max-w-xl">` dentro la stessa `Section`, senza più il `grid lg:grid-cols-[minmax(0,1fr)_minmax(0,360px)]` né il secondo `Reveal`. Email e indirizzo restano comunque raggiungibili altrove sul sito (Footer, colonna Azienda e riga legale — vedi §5), quindi nessuna informazione è andata persa, solo la sua duplicazione su questa pagina.
