...
- **Footer**: scuro (`--bg-black`), 3 colonne (brand + azienda + nav), riga legale. Struttura riconciliata con le pagine reali il 2026-08-17: **Navigazione** rispecchia esattamente le 5 voci dell'header (`Header.tsx`, stesso ordine e stessa label "Perchè AXON") + Shop; **Azienda** è Contatti (`/contatti`) + `info@axon-tech.it` (mailto) + la ragione sociale "Axon-Tech S.r.l." come testo semplice (non è più un link `#`: non ha una destinazione); riga legale Privacy/Cookie/Termini punta a `/privacy` `/cookie` `/termini` via `next/link` invece dei vecchi stub `#`. La colonna **Prodotto** (AXON KIT/Come si usa/FAQ/Testimonianze) è stata rimossa il 2026-08-17 — nessuna voce esclusiva che non fosse già raggiungibile altrove — e la griglia è scesa da `lg:grid-cols-4` a `lg:grid-cols-3`, con Navigazione spostata al suo posto (ordine finale: brand, azienda, navigazione). Icone social: rese solo quando `SOCIAL.<piattaforma>` in `lib/links.ts` non è più `'#'` (rendering condizionato sul valore, non commentato/hardcoded) — finché tutti e tre restano placeholder, il blocco social non renderizza nulla, evitando link morti; impostare l'URL reale in `lib/links.ts` è un one-line restore, nessuna modifica a `Footer.tsx`.
