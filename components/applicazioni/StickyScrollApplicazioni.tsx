...
import { Container, MediaFrame, Reveal } from "@/components/ui";
...
export default function StickyScrollApplicazioni() {
  ...
  return (
    <>
      {/* ── Sotto lg: swipe deck, confermato dal prototipo al posto dello
          scroll-jacking (v. "StickyScroll → swipe deck (confermato)").
          Zero position:sticky, zero window.scrollTo, zero contenitore
          fittizio da 700vh: il deck scorre in normal flow con snap
          orizzontale nativo. Testo sempre aperto (niente accordion), card
          4:3 (non 1:1 — un riquadro quadrato a 375px costerebbe 327px di
          altezza per sola decorazione). Il titolo, sr-only sul desktop
          perché il pin non lasciava spazio, torna visibile. */}
      <section className="lg:hidden bg-black" style={{ paddingTop: 'var(--section-y)', paddingBottom: 'var(--section-y)' }}>
        <Container className="mb-5">
          <Reveal>
            <h2 className="text-h2 mb-2.5">Contesti d&rsquo;uso</h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p
              className="text-[1.125rem] md:[font-size:var(--fs-lead)]"
              style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}
            >
              Sette momenti della giornata in cui AXON lavora con te.
            </p>
          </Reveal>
        </Container>
            ...
            >
              <div className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden border border-white/10">
                ...
                <div
                  ...
                  style={{ backgroundImage: 'linear-gradient(to top, rgba(0,0,0,0.4), transparent 60%)' }}
                />
              </div>
        ...
        <p
          className="mt-[18px] px-6 md:px-12 flex items-center gap-2"
          style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)' }}
        >
          Scorri per i 7 contesti <ArrowRight size={14} strokeWidth={2} aria-hidden="true" />
        </p>
      </section>
