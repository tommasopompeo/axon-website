...
// ── Evidenze pilota Fenzi — headline da evidence.md ──
const evidenze = [
  { value: '+28%', label: 'equilibrio (scala Tinetti)', note: 'p = 0,0024' },
  { value: '+42%', label: 'nella salita delle scale', note: 'p = 0,0025' },
  { value: '+30%', label: 'forza di presa', note: 'p < 0,001' },
]

// ── Perché B2B ──
const perche = [
  { icon: ShieldCheck, title: 'Passivo e sicuro', text: 'Nessuna batteria, nessuna elettronica, nessuna manutenzione.' },
  { icon: Layers, title: 'Integrabile', text: 'Si applica e si cuce su tessuti, calzature e accessori.' },
  { icon: EyeOff, title: 'Discreto', text: 'Invisibile a chi lo indossa.' },
  { icon: Stethoscope, title: 'Dispositivo medico Classe I', text: 'Certificato, non invasivo, passivo.' },
]
...
export default function AziendeContent() {
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-5 max-w-3xl">
            ...
            <Reveal trigger="mount" delay={0.12}>
              <p style={{ fontSize: 'var(--fs-lead)', color: 'var(--text-muted)', lineHeight: 1.6 }}>
                Una tecnologia orizzontale, integrabile in prodotti, percorsi e team.
                Dalla moda allo sport, dalla sanità al benessere sul lavoro.
              </p>
            </Reveal>
            <Reveal trigger="mount" delay={0.2}>
              <Button href="#contatto-aziende" variant="primary" size="lg">
                Parla con noi
              </Button>
            </Reveal>
          </div>
      ...
      {/* ── Casi d'uso ── */}
      <Section id="casi-uso" elevated className="!pt-0">
        <Container>
                ...
                <RevealItem key={title}>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      <h3 className="font-semibold" style={{ fontSize: 'var(--fs-h3)', letterSpacing: '-0.01em' }}>
                        {title}
                      </h3>
                      <p style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', lineHeight: 1.55 }}>
                        {text}
                      </p>
                    </div>
                  </div>
        ...
        <Container>
          <div className="flex flex-col gap-12 lg:gap-16">
            <div className="flex flex-col gap-4 max-w-2xl">
              <Reveal>
                <h2 className="font-bold" style={{ fontSize: 'var(--fs-h2)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
                  Primi dati dal campo.
                </h2>
              </Reveal>
              <Reveal delay={0.12}>
                <p style={{ fontSize: 'var(--fs-lead)', color: 'var(--text-muted)', lineHeight: 1.6 }}>
                  Studio pilota condotto presso l'IPAB F. Fenzi di Conegliano su 14 residenti.
                  Tutti gli outcome statisticamente significativi (p&nbsp;&lt;&nbsp;0,05).
                </p>
              </Reveal>
            </div>
                ...
                <RevealItem key={label}>
                  ...
                  >
                    <p
                      ...
                      style={{ fontSize: 'var(--fs-stat)', color: 'var(--brand)', lineHeight: 1, letterSpacing: '-0.02em' }}
                    >
                      {value}
                      <span style={{ fontSize: '1.25rem', color: 'var(--text-subtle)' }}>*</span>
                    </p>
                    <p className="font-semibold" style={{ fontSize: 'var(--fs-h3)' }}>
                      {label}
                    </p>
            ...
            <Reveal delay={0.1}>
              <p style={{ fontSize: 'var(--fs-caption)', color: 'var(--text-subtle)', lineHeight: 1.55 }}>
                * Studio pilota IPAB F. Fenzi, 14 soggetti. Risultati preliminari, confronto pre/post nello stesso soggetto.
                Non generalizzabili a popolazioni diverse senza ulteriori studi.
              </p>
            </Reveal>
          </div>
        </Container>
      ...
      {/* ── Perché B2B ── */}
      <Section id="perche-b2b" elevated>
        <Container>
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,400px)_minmax(0,1fr)] lg:gap-20 lg:items-start">
            <div className="flex flex-col gap-5">
              <Reveal>
                <h2 className="font-bold" style={{ fontSize: 'var(--fs-h2)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
                  Perché Axon per il B2B.
                </h2>
              </Reveal>
              <Reveal delay={0.12}>
                <p style={{ fontSize: 'var(--fs-lead)', color: 'var(--text-muted)', lineHeight: 1.6 }}>
                  Un dispositivo medico progettato per integrarsi, non per sovrapporsi.
                </p>
              </Reveal>
            </div>

            <RevealGroup className="grid grid-cols-1 sm:grid-cols-2 gap-5" staggerDelay={0.08}>
              {perche.map(({ icon: Icon, title, text }) => (
                <RevealItem key={title}>
                  ...
                  >
                    <span
                      className="inline-flex items-center justify-center"
                      style={{ width: 40, height: 40, borderRadius: '999px', background: 'var(--brand-soft)', color: 'var(--brand)' }}
                    >
                      <Icon size={18} strokeWidth={2} aria-hidden="true" />
                    </span>
                    <h3 className="font-semibold" style={{ fontSize: 'var(--fs-h3)' }}>{title}</h3>
                    <p style={{ fontSize: 'var(--fs-body)', color: 'var(--text-muted)', lineHeight: 1.55 }}>{text}</p>
                  </div>
                </RevealItem>
              ))}
            </RevealGroup>
